...
usage() {
    ...
    echo "  add-user <site_key> <user> <password>  Add a user (auto-hashes if needed)"
...
hash_password() {
    ...
    if [ ! -f "$GOLYN_BIN" ]; then
        ...
        if command -v go >/dev/null 2>&1; then
            ...
            (cd ../.. && go build -o golyn cmd/golyn.go)
            GOLYN_BIN="../../golyn"
        else
...
case "$1" in
    ...
    add-user)
        ...
        run_sql "INSERT INTO auth.users (site_id, username, password_hash) VALUES ('$SITE_ID', '$3', '$PASS');"
        ;;
    del-user)
        if [ -z "$2" ]; then
            usage
...
# ./manage_clients.sh add-user <sitio> <email> <hash>: Crea un usuario vinculado a un sitio. Requiere el hash Bcrypt de la password.
